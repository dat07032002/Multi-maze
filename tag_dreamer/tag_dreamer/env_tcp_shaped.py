    ...
    def __init__(
        ...
        if PolicyCameraModel is not None and CameraConfig is not None:
            try:
                ...
                print("[TCP ENV SHAPED] Policy image: synthetic sim-matched patch")
            except Exception as exc:
                print(f"[TCP ENV SHAPED] Policy image: bridge camera fallback ({exc})")
        else:
            print(
                "[TCP ENV SHAPED] Policy image: bridge camera fallback "
                "(tag_mujoco unavailable)"
            )
