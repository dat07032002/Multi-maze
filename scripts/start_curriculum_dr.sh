...
source_checkpoint="${1:?Pass the source checkpoint as argument 1.}"
source_run="${2:?Pass the source run directory as argument 2.}"
python_bin="${TAG_PYTHON:-python}"
run_id="${TAG_RUN_ID:-holeaware_curriculum_dr_100k_$(date +%Y%m%d_%H%M%S)}"
...
export TAG_TRAINING_PROFILE=tag_sim_v2_holeaware_curriculum_dr
...
echo "Curriculum DR launched."
echo "RUN_DIR=$run_dir"
echo "Attribution command after a checkpoint is saved:"
echo "bash $repo_root/scripts/run_dr_attribution.sh CHECKPOINT $run_dir/config.yaml OUTPUT STEP"
